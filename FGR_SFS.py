...
def FGR_SFS(X, Y, M, gamma, lambda_lsr, k, r, l, random_state=None, hard_thre=False):
    ...
    for iter_ in range(max_iter):
        ...
        P = np.linalg.inv(W.T @ XX @ W) @ W.T @ XY

        # update W
        WW = W @ W.T
        MW = M @ W
        W_down = M @ WW @ MW + lambda_lsr * XX @ W @ P @ P.T + lambda_w * WW @ W
        temp = (MW @ B + lambda_w * W + lambda_lsr * XY @ P.T) / W_down
        W *= temp
